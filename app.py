...
# GitHub URL where the model is hosted (must be uploaded as a raw file)
MODEL_URL = "https://github.com/snehuuu28/Life_Expectancy-Prediction/raw/main/best_model.pkl"
...
def load_model():
    try:
        ...
        if response.status_code == 200:
            ...
            st.error("🚨 Error: Could not download the model from GitHub!")
            return None
...
if prediction is not None:
    ...
    st.download_button("📂 Download Prediction as CSV", results.to_csv(index=False).encode('utf-8'), "prediction.csv", "text/csv")
